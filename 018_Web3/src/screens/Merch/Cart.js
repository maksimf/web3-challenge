...
const Cart = ({ cartNum, setCartNum }) => {
  ...
  const handleRemove = async (sizeId) => {
    try {
      const url =
        BASEURL + "/cart/remove?sizeId=" + sizeId + "&userId=" + userId;
  ...
  return (
    <div className="flex flex-col">
      ...
      {publicKey ? (
                ...
                cartList.map((item, index) => (
                  <div key={index} className="w-full flex p-2 m-2">
                    <div className="w-32 mr-4">
                      <img className="w-full" src={item.imageUrl[0]} alt="" />
                    </div>
                      ...
                      >
                        ...
                        <div className="text-md mb-4 float-right">
                          <Select
                            native
                            value={item.amount}
                            onChange={(e) => amountChange(e, item._id)}
                          >
              ...
              <div className="w-full md:w-3/12 md:ml-8 px-4 flex flex-col mb-8">
                ...
                <div className="mb-8 font-bold">
                  ...
                  <span className="float-right">
                    {priceList.map((item, index) => (
                      <div key={index} className="">
                        {item.currency === "Sol" ? item.price + 1 : item.price}{" "}
                        {item.currency}
                      </div>
                    ))}
                  </span>
        ...
        <p className="text-center text-bold my-8 text-xl">
